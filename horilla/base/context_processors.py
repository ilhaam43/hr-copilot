...
def get_custom_available_languages(request):
    """
    Custom context processor to provide only English and Indonesian languages
    instead of all system available languages.
    """
    from django.conf import settings
    
    # Only return the languages defined in settings.LANGUAGES
    custom_languages = getattr(settings, 'LANGUAGES', [])
    return {"CUSTOM_LANGUAGES": custom_languages}
