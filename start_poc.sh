...
need() { command -v "$1" >/dev/null || { echo "ERROR: '$1' tidak ditemukan."; exit 1; }; }
...
embed_len() {
  local model="$1"
  local json
  json="$(curl -s http://127.0.0.1:11434/api/embed \
            -H 'Content-Type: application/json' \
            -d "{\"model\":\"${model}\",\"input\":\"dim check\"}")"
  echo "$json" > "$LOG_DIR/emb_check_${model//[:\/]/_}.json"
  # format resmi: {"embeddings":[ [..vector..] ]}  → ambil embeddings[0]
  # fallback lama: {"embedding":[..]} atau {"data":[{"embedding":[..]}]}
  local len
  len="$(echo "$json" | jq '((.embeddings[0]) // .embedding // (.data[0].embedding)) | length' 2>/dev/null || echo 0)"
  echo "${len:-0}"
}
...
if [[ "$EMBLEN" -eq 0 ]]; then
  ...
  if [[ "$TESTLEN" -gt 0 ]]; then
    EMBED_MODEL="mxbai-embed-large"; EMBLEN="$TESTLEN"
    echo "   ✓ Fallback OK: EMBED_MODEL=$EMBED_MODEL (dim=$EMBLEN)"
  else
    echo "ERROR: kedua model mengembalikan embedding 0."
    echo "  Debug JSON: $LOG_DIR/emb_check_${EMBED_MODEL//[:\/]/_}.json & $LOG_DIR/emb_check_mxbai-embed-large.json"
    exit 1
  fi
else
